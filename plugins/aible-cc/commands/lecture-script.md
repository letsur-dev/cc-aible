...
- 모든 `scripts/{n}_script.md` 파일이 존재하는지
- 각 파일이 비어 있지 않은지
...
1. `lecture-script-reviewer` 에이전트를 호출하세요:
   ...
   - prompt에 포함: 슬라이드 설명 파일 경로(근거) + 스크립트 파일 경로(평가 대상) + 강의 맥락
...
6. **미통과 & 시도 횟수 남음**:
   ...
   - prompt에 포함: 강의 맥락 + **최신 스크립트만** + 리뷰어 피드백 전문 + 슬라이드 설명 파일 경로
...
1. `lecture-script-red-team` 에이전트를 호출하세요:
   ...
   - prompt에 포함: 슬라이드 설명 파일 경로 + 스크립트 파일 경로 + 리뷰 점수 + 강의 맥락
